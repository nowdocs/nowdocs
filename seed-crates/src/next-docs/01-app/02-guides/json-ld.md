...
Our current recommendation for JSON-LD is to render structured data as a `` tag in your `layout.js` or `page.js` components.
...
> **Good to know**: The `next/script` component is optimized for loading and executing JavaScript. Since JSON-LD is structured data, not executable code, a native `` tag is the right choice here.
