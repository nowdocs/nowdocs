...
- **模型格式**：safetensors，`DType::F16` 加载（砍一半 RSS，精度对 cosine 无影响），mmap
...
| §2.2 Embedder | 数字修正：`<5ms`→~5-40ms 无感；`<50MB RAM`→增量<50MB/**常驻~1.2-1.5GB（§6.5）**；模型 ~66MB F16（F32 RSS ~131MB）；加 `DType::F16`；加模型完整性 pin（§6.3）+ 缓存路径 nowdocs（A4） |
