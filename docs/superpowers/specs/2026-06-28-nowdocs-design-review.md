...
> **核实方法**：4 个研究子代理 + GitHub API 坐实 + Context7 deep-research（103-agent workflow + 9 维度定向深挖）+ 源码级核实（lancedb 0.31 / lance 8.0 / candle / ort / MCP spec）。
...
- lancedb 0.31.0（2026-05-28）原生 `query().full_text_search().nearest_to().rerank(RRFReranker).execute_hybrid()` 一条链 = 向量 + BM25 + RRF；tantivy 已是其传递依赖。
...
**数据库**：lancedb 0.31 单依赖，每 docset 一张表 `~/.cache/nowdocs/db/<docset>.lance`，install 时拉取整个表文件。
...
- 拒绝 `next`/不稳定格式版本。FTS 索引约束 `use_tantivy=True` 拒绝项在 lancedb 0.31 已自动满足（旧 tantivy backend 移除，`Index::FTS` 只剩原生 Lance inverted index，无 `use_tantivy` 字段——见附录 §G）。
...
> **实现核实（2026-06-29，lancedb 0.31 源码）**：`use_tantivy` 字段在 0.31 已删除——`Index::FTS(FtsIndexBuilder::default())` 其中 `FtsIndexBuilder` = `lance_index::scalar::InvertedIndexParams`（lancedb `src/index/scalar.rs:54` re-export），`.default()` 即原生 Lance inverted index，无 tantivy backend 可选。本约束（B1）因此**已自动满足**，CI 闸门无需额外检查该字段（属空操作）。详见附录 §G。

### 6.5 C1 内存（澄清，非炸弹）
**纠正子代理误报**：所谓"每个 Table 默认 6GiB 索引缓存，50 docset = 350GiB"是**幻觉**。源码核实（lance 8.0 `dataset.rs:110`）：
```
...
- **未核实风险**：lancedb 0.31 per-table 自定义 tokenizer API 待实现时核源码；若不支持 → CJK 暂走纯向量（无 BM25）或等 lancedb 支持。
...
5. **依赖解析版本（1a 实现核实，cargo 1.93 / 2026-06-29 解析）**：plan 起点版本因跨依赖 `half` 冲突（lancedb 0.18 锁 `half =2.4.1` vs candle 0.9 需 `half ^2.5`）无法共存，已统一升到最新兼容：`lancedb 0.31` / `candle-core 0.11`（default-features=false）/ `candle-nn 0.11` / `candle-transformers 0.11` / `tokenizers 0.23` / `tiktoken-rs 0.12` / `hf-hub 0.5`（2a 升级，修 XET redirect；见附录 §H）。`clap 4.5` / `serde 1.0` / `anyhow 1.0` / `thiserror 1.0` / `regex 1.10` / `sha2 0.10` / `dirs 5.0` 维持 plan 约束。换依赖/换模型不在此列（架构级，需 Main 决策）；此处仅同依赖的版本号核实。
...
### G. lancedb 0.31 FTS + hybrid API 核实（Task 2b 实现）
*API compatibility verified by `cargo check` on rustc 1.97.0, 2026-07-10.*

**Main 源码级核实（2026-06-29，`~/.cargo/registry/src/index.crates.io-.../lancedb-0.31.0/` + `lance-index-8.0.0/`）**：
...
- **FTS 索引**：`table.create_index(&["text"], Index::FTS(FtsIndexBuilder::default())).execute().await`。`FtsIndexBuilder` = `lance_index::scalar::InvertedIndexParams`（`lancedb/src/index/scalar.rs:54` re-export）。**`use_tantivy` 字段在 0.31 已删除**——旧 tantivy backend 完全移除，只剩原生 Lance inverted index（§6.4 B1 约束已自动满足，空操作）。
- **`FullTextSearchQuery`**：在 `lance-index-8.0.0/src/scalar.rs:302-382`（**非 lancedb crate**）。`::new(String)` 只传 query 文本，列名从 FTS 索引推断（`.with_column()` 可选）。
...
> - `FixedSizeListArray::try_new_from_values` 需 `lance-arrow = "8"` 的 `FixedSizeListArrayExt` trait（非 arrow 原生）。
...
### H. hf-hub 0.5 API 核实（Task 2a 实现）
...
- **fetch_by_idx scalar filter API**：lancedb 0.31 `table.query().only_if("chunk_idx IN (1,2,3)").execute()` 返回 `SendableRecordBatchStream`，需 `use lancedb::query::ExecutableQuery` 导入 `execute()` 方法。column 解析时 `column_by_name()` 返回 `Option<&Arc<dyn Array>>`（非 `&dyn Array`），closure 参数类型需匹配。提取为 `parse_search_hits` / `parse_search_hits_with_score` 两个 helper 复用于 `fetch_by_idx` 和 `hybrid_search`。
