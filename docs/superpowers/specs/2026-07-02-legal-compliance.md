...
1. **政策适用范围**：nowdocs 公共 registry（当前 `github.com/nowdocs-registry/*`，未来 `registry.nowdocs.rs/*`）
2. **举报渠道**（走 GitHub 内置）：
...
2. **三类网络行为**（用户主动触发）：
   ...
   - `nowdocs install` / `update`：从 registry 下载 docset（白名单 `github.com/nowdocs-registry/*`、`registry.nowdocs.rs/*`）
...
6. **registry 网站隐私**（未来）：若 `registry.nowdocs.rs` 提供网页，将另行声明（当前 registry 走 GitHub，适用 GitHub 隐私政策）
...
3. `registry.nowdocs.rs` 域名是否已持有？（影响 Privacy / AUP 中"未来自建 registry"措辞——当前仅 GitHub）
