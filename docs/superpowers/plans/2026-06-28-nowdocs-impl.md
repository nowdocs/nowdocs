...
- **5d seed crates**: Next.js(MIT) / React(CC-BY-4.0) / Vue(CC-BY-4.0) — verify license per §6.10 before publish; Astro(MIT) reserve.
  - **DONE**: 3 share artifacts in `seed-crates/share/{nextjs-docs,react-docs,vue-docs}-share/<docset>/` — each `manifest.json` (model-locked, license-allowlisted, attribution set for CC-BY) + `chunks.jsonl` (text only, no vectors, no images) + `LICENSE` + `NOTICES`. Source upstream repos cloned shallow, MDX/JSX stripped via `seed-crates/tmp/prep_docs.py`, manifests patched for per-docset legal metadata via `seed-crates/tmp/patch_manifest.py`. CI: `ci-check-manifest.sh` PASS for all 3; `ci-no-vectors.sh` CLEAN. Counts: nextjs-docs 437 files / 5791 chunks; react-docs 220 files / 4360 chunks; vue-docs 118 files / 3280 chunks. **Open**: `ingest::build_manifest` hardcodes `legal.license="MIT"` (src/ingest.rs:111) — manifests are post-patched via `patch_manifest.py`; an ingest CLI flag (e.g. `--license MIT|Apache-2.0|CC-BY-4.0 --copyright-holder … --attribution …`) would eliminate the post-patch step. Listed for Task 5d/4d owner.
