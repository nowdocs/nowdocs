...
fn test_invalid_archive_install_leaves_no_active_manifest_or_store() {
    ...
    assert!(!db.exists(), "invalid install should not create db directory");
}
...
fn test_failed_update_preserves_old_active_manifest_and_store() {
    ...
    unsafe { std::env::set_var("NOWDOCS_TEST_URL", format!("file://{}", bad_tar_path.display())) };
    ...
    let raw = std::fs::read_to_string(&mp).unwrap();
    let m = manifest::parse_manifest(&raw).unwrap();
    assert_eq!(m.doc_version, "1.0.0", "old manifest should be preserved after failed update");
    
    // Old store should still be accessible
    let store = Store::open(docset).unwrap();
    let chunks = store.dump_chunks().unwrap();
    assert_eq!(chunks.len(), 2, "old store should be preserved after failed update");
}
...
"#;
    ...
    // Update to v2
    unsafe { std::env::set_var("NOWDOCS_TEST_URL", format!("file://{}", v2_tar_path.display())) };
    nowdocs::registry::update(docset).unwrap();
...
fn test_stale_staging_detection() {
    ...
    assert_eq!(staging_dirs.len(), 1, "should detect one stale staging directory");
    assert_eq!(staging_dirs[0], stale_dir, "should detect the correct stale staging directory");
}
